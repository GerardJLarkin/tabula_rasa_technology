...
#     ret, frame = cap.read()
#     # Alternative (Using Bitwise Shift for True RGB Encoding) true color encoding (like a packed integer in 24-bit RGB format), you can use bitwise operations:
#     frame = (frame[..., 0] << 16) | (frame[..., 1] << 8) | frame[..., 2]
#     # normalise values between -1 and 1
#     frame = (frame - frame.min()) / (frame.max() - frame.min()).astype('float32')
#     print(frame)
#     frame_grab = datetime.datetime.now()
#     print(frame_grab - now)
...
@timeit
def run(cam: VideoStreamFFmpeg, run_task: bool): #VideoStreamCV | 
    timer = []
    for _ in range(100):
        t0 = time.perf_counter()
        cam.read()
