...
# @njit(nopython=True, fastmath=True, parallel=True) #numba.float32(numba.types.Array(numba.float32, 2, "C")), 
...
def patoms2d(single_frame_array, frame_ind):
    ...
    for i in chunks:
        ...
        pattern_centroid_x = np.array([np.floor(x_vals.mean())] * pat_len).reshape(pat_len,1)
        pattern_centroid_y = np.array([np.floor(y_vals.mean())] * pat_len).reshape(pat_len,1)
