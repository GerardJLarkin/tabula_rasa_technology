...
def patoms2d(x_len, y_len, single_frame_array, frame_ind):
    ...
    for patom_ind, pat in enumerate(s_res):
        ...
        mean_x = sum(x)/pat_len
        x_z = [i - mean_x for i in x]; min_x = min(x_z); max_x = max(x_z)
        norm_x = [(i - min_x)/(max_x - min_x) for i in x_z]
        y = [p[1][1] for p in pat]
        mean_y = sum(y)/pat_len
        y_z = [i - mean_y for i in y]; min_y = min(y); max_y = max(y)
        norm_y = [(i - min_y)/(max_y - min_y) for i in y_z]

        pattern_centroid_x = sum(norm_x)/pat_len
        #pattern_centroid_x = [pattern_centroid_x] * pat_len
        pattern_centroid_y = sum(norm_y)/pat_len
        #pattern_centroid_y = [pattern_centroid_y] * pat_len

        #patom_ind = [patom_ind] * pat_len
        #frame_ind = [frame_ind] * pat_len
        
        patom_time = clock_gettime_ns(CLOCK_REALTIME) 
        #patom_time = [patom_time] * pat_len
        #patom = [[norm_x, norm_y], [pattern_centroid_x, pattern_centroid_y], patom_ind, frame_ind, patom_time]
        patom = [norm_x, norm_y, pattern_centroid_x, pattern_centroid_y, patom_ind, frame_ind, patom_time]
        norm_patoms.append(patom)

        ## limit to pattern centroid and list of norm distances
        ## when creating a reference table of the archetype of similar patterns I need the average of the centroids 
        # and the average number of pixels (rows) and the average distance of all the points to the centroid 
