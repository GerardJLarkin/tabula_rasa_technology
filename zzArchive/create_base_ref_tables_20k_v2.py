...
con2d = sqlite3.connect("ref2d_v2.db")
cur2d = con2d.cursor()
...
def ref_patoms2d(x_len, y_len, single_frame_array, frame_ind):
    ...
    for patom_ind, pat in enumerate(s_res):
        ...
        patom_len = np.array([pat_len] * pat_len).reshape(pat_len,1)
        ...
        patom_array = np.hstack([norm_x, norm_y, pattern_centroid_x, pattern_centroid_y, patom_len, patom_ind, frame_ind_arr, patom_time]).astype(np.float32)
...
for i in range(100):
    ...
    for j in range(num_patoms):
        ...
        cur2d.execute(f"CREATE TABLE pat_2d_ref_{table_num}(x_pos_dist, y_pos_dist, x_cent, y_cent, patom_len, patom_ind, frame_ind, patom_time);")
        cur2d.executemany(f"INSERT INTO pat_2d_ref_{table_num}(x_pos_dist, y_pos_dist, x_cent, y_cent, patom_len, patom_ind, frame_ind, patom_time) VALUES (?,?,?,?,?,?,?,?)", patom)
        table_num_seq += 1 
