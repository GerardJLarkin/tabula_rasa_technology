...
static int *dsu_parent;
static int *dsu_rank;

static inline int dsu_find(int x) {
...
void* producer(void *_) {   
    ...
    unsigned n;
    snd_pcm_t *cap;
    if (init_v4l2(&vfd, &bufs, &n) < 0) fprintf(stderr, "ERROR: cannot open %s\n", VIDEO_DEVICE); return NULL;
    if (init_alsa(&cap) < 0) fprintf(stderr, "ERROR: cannot open ALSA device %s\n", AUDIO_DEVICE); return NULL;
    ...
    while (keep_running) {
        ...
        threshold_group(norm, FRAME_WIDTH, FRAME_HEIGHT, THRESHOLD, labels);
        // After threshold_group(norm, FRAME_WIDTH, FRAME_HEIGHT, THRESHOLD, labels);

        // Number of pixels
        int N = FRAME_WIDTH * FRAME_HEIGHT;

        // 1) Accumulate sums and counts per label
        float *sum_i = calloc(N, sizeof(float));
        float *sum_j = calloc(N, sizeof(float));
        ...
        // 2) Compute centroids
        float *cent_i = malloc(N * sizeof(float));
        float *cent_j = malloc(N * sizeof(float));
        for (int lbl = 0; lbl < N; lbl++) {
            if (count[lbl] > 0) {
                cent_i[lbl] = sum_i[lbl] / count[lbl];
                cent_j[lbl] = sum_j[lbl] / count[lbl];
            }
        }

        // 3) Find maximum deviation per group
        float *max_dev = calloc(N, sizeof(float));
        for (int idx = 0; idx < N; idx++) {
            int lbl = labels[idx];
            float di = (idx % FRAME_WIDTH) - cent_i[lbl];
            float dj = (idx / FRAME_WIDTH) - cent_j[lbl];
            float dev = fmaxf(fabsf(di), fabsf(dj));
            if (dev > max_dev[lbl]) max_dev[lbl] = dev;
        }

        // 4) Allocate & fill normalized coordinates (half-float)
        pck.loc_i = malloc(N * sizeof(uint16_t));
        pck.loc_j = malloc(N * sizeof(uint16_t));
        for (int idx = 0; idx < N; idx++) {
            int lbl = labels[idx];
            float di = ((idx % FRAME_WIDTH) - cent_i[lbl]) / max_dev[lbl];
            float dj = ((idx / FRAME_WIDTH) - cent_j[lbl]) / max_dev[lbl];
            pck.loc_i[idx] = float_to_half(di);
            pck.loc_j[idx] = float_to_half(dj);
        }
...
void* consumer(void *_) {
    ...
    while (keep_running || qc > 0) {
        ...
        sendto(s, p.audio, asz, 0,
               (struct sockaddr*)&d, sizeof(d));
        // send loc_i buffer (normalized i-coordinates)
        uint32_t lisz = FRAME_WIDTH*FRAME_HEIGHT * sizeof(uint16_t);
        sendto(s, &lisz, sizeof(lisz), 0, (struct sockaddr*)&d, sizeof(d));
        sendto(s, p.loc_i, lisz,       0, (struct sockaddr*)&d, sizeof(d));

        // send loc_j buffer (normalized j-coordinates)
        uint32_t ljsz = FRAME_WIDTH*FRAME_HEIGHT * sizeof(uint16_t);
        sendto(s, &ljsz, sizeof(ljsz), 0, (struct sockaddr*)&d, sizeof(d));
        sendto(s, p.loc_j, ljsz,       0, (struct sockaddr*)&d, sizeof(d));
