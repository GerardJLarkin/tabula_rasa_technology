...
// -------------------------------------------------------------------------
// PART 3: Frame Processing - Create Groups and Print Summary
// -------------------------------------------------------------------------
...
void process_frame_example(unsigned int *color_array, int width, int height,
    ...
    // First pass: determine max_label.
    for (int i = 0; i < num_pixels; i++) {
        if (labels[i] > max_label)
            max_label = labels[i];
    }

    // Allocate an array to count pixels per group.
    ...
    // Second pass: count pixels for each group.
    for (int i = 0; i < num_pixels; i++) {
        group_counts[labels[i]]++;
    }

    // Now process groups that have at least 10 pixels.
