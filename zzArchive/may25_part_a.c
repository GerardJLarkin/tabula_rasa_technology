 ...
 */
 ...
 void* producer(void *_) {
     ...
     while (keep_running) {
         ...
         // grouping: label connected pixels and prepare per-group normalization
         int *labels = malloc(FRAME_WIDTH * FRAME_HEIGHT * sizeof(int));
         int group_count = threshold_group(norm, FRAME_WIDTH, FRAME_HEIGHT, THRESHOLD, labels);
         // compute centroids and max distances
         int *counts   = calloc(group_count, sizeof(int));
         float *sum_x  = calloc(group_count, sizeof(float));
         float *sum_y  = calloc(group_count, sizeof(float));
         ...
         float *centroid_x = malloc(group_count * sizeof(float));
         float *centroid_y = malloc(group_count * sizeof(float));
         for (int g = 0; g < group_count; g++) {
             centroid_x[g] = sum_x[g] / counts[g];
             centroid_y[g] = sum_y[g] / counts[g];
         }
         float *max_dx = calloc(group_count, sizeof(float));
         float *max_dy = calloc(group_count, sizeof(float));
         for (int y = 0; y < FRAME_HEIGHT; y++) {
             for (int x = 0; x < FRAME_WIDTH; x++) {
                 int idx = y * FRAME_WIDTH + x;
                 int g = labels[idx];
                 float dx = fabsf(x - centroid_x[g]);
                 float dy = fabsf(y - centroid_y[g]);
                 if (dx > max_dx[g]) max_dx[g] = dx;
                 if (dy > max_dy[g]) max_dy[g] = dy;
             }
         }
         // write per-group member info to SD card
         char filename[128];
         snprintf(filename, sizeof(filename), "/home/pi/group_snapshot_%u.txt", pck.seq_id);
         FILE *f = fopen(filename, "w");
         if (!f) fprintf(stderr, "Error opening %s: %s", filename, strerror(errno));
         for (int g = 0; g < group_count; g++) {
             if (f) fprintf(f, "Group %d centroid=(%.2f,%.2f) members=%d", g, centroid_x[g], centroid_y[g], counts[g]);
             printf("Group %d centroid=(%.2f,%.2f) members=%d", g, centroid_x[g], centroid_y[g], counts[g]);
             for (int y = 0; y < FRAME_HEIGHT; y++) {
                 for (int x = 0; x < FRAME_WIDTH; x++) {
                     int idx = y * FRAME_WIDTH + x;
                     if (labels[idx] != g) continue;
                     float dx = x - centroid_x[g];
                     float dy = y - centroid_y[g];
                     float inorm = (max_dx[g] > 0) ? dx / max_dx[g] : 0.0f;
                     float jnorm = (max_dy[g] > 0) ? dy / max_dy[g] : 0.0f;
                     float val = norm[idx];
                     if (f) fprintf(f, "(%d,%d) val=%.6f inorm=%.3f jnorm=%.3f", x, y, val, inorm, jnorm);
                     printf(" (%d,%d) val=%.6f inorm=%.3f jnorm=%.3f", x, y, val, inorm, jnorm);
                 }
             }
             if (f) fprintf(f, "");
         }
