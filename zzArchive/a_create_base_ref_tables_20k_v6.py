...
# flatten 3 channel image (take a single channel)
...
for i in range(0, 900, 6):
    ...
    # create empty list to append arrays
    # add random choice selector here for different paths
    # if time_int == 0:
    pad_array = np.pad(flat_circle, ((top-i, bottom+i), (left+i, right-i)), 'constant', constant_values=(0)) # top, bottom, left, right
...
frame_seq = 0
patoms = []
for i in padded_arrays[0]:
    # create data
    frame = i
    x_len = frame.shape[0]
    y_len = frame.shape[1]
    characters = string.ascii_letters + string.digits
    patom_id = ''.join(random.choice(characters) for i in range(6))
    if frame_seq == 0:
        frame_seq = 1
    elif frame_seq == 1:
        frame_seq = 0
    frame_patoms = ref_patoms2d(x_len, y_len, frame, frame_seq)
    for ind, patom in enumerate(frame_patoms):
        if ind == 0:
