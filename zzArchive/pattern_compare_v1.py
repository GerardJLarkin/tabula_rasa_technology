...
compare_list = list(product(files, files))[:1000]
    ...
    with open(f'/home/gerard/Desktop/capstone_project/patoms/{i[1]}', 'r') as f2:
        ...
        if (nxc_diff + nyc_diff + nzc_diff) <= centroid_diff:
            if shape1 <= shape2:
                ...
                sim_perc = sim_sum/shape2
                if sim_perc >= sim_threshold:
                    similar_patterns.append((filtered_lists[ind], ind, sim_perc))
            else:
...
e = perf_counter()
print("Time for 1 sec new data and compare in seconds is: ", (e-s)/60)
