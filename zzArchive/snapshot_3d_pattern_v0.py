...
def snapshot_pattern(array):
    ...
    for ind, pat in enumerate(s_res):
        ...
        centroid_list = [tuple([0.0]+centroid+[0.0])]
        loc = [p[1] for p in pat]
        #print(loc)
        dist = list(map(lambda x: math.dist(centroid, list(x)), loc)) # heavy 32.5 Mib
        #print(len(dist))
        tot = sum(dist)
        #print(tot)
        norm_dist = [i/tot for i in dist]
        # WHAT ARE THESE NEXT 2 LINES DOING? is this already part of an attempt to get a normalised location?
        ## why am I getting a negative centroid?
        neg_centroid = [-i for i in centroid]
        norm_loc = list(map(lambda i, j: sum(i, j), loc, neg_centroid))
        #print(norm_dist)
        ...
        # print(type(norm_dist),' norm_dist')
        ## expand this to include, centroid and pixel indices when centroid is translated to the origin
        patom = list(zip(val, x, y, z, norm_dist)) #.extend(centroid) # heavier than expected 89 MiB #centroid[0], centroid[1], centroid[2], 
