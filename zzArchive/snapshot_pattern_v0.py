...
operating_time = strftime("%Y-%m-%dT%H:%M:%S.%f", localtime())
...
def snapshot_pattern(data, timestamp):
    threshold = 0.0001
    orig_arr = array
    ...
    for pat in s_res:
        ...
        patom = list(zip(val, loc, norm))
        norm_dist_patoms.append((patom, timestamp))

    return norm_dist_patoms



identified_patterns = snapshot_pattern(array, timestamp)
print(identified_patterns[0])
