...
files = [np.load(f) for f in glob.glob(os.path.join(folder, '*.npy'))]
print('loaded')
sim_threshold = 0.25
...
with open('pairs.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['id1', 'id2'])        # optional header

    for idx1, idx2 in combinations(range(len(files)), 2):
        id1, id2, sim = compare(files[idx1], files[idx2])
...
# ### updated more efficient version (apparently)
# import numpy as np
# import os, glob
# from time import perf_counter
# import sys
...
# from tabula_rasa_technology.simple_approach.compare import compare
# from tabula_rasa_technology.simple_approach.patom_groups import group_arrays, load_memmap

# # perform the compare only and write output to disk
...
#     # 2) Use the optimized grouping
#     groups = group_arrays(file_paths, compare, sim_threshold)

#     print(f'Building {len(groups)} reference patoms…')
#     for group in groups:
#         # 3) Load via the same cached memmap helper
#         patoms = [load_memmap(p) for p in group]

#         # 4) Split rows & stack in one go
#         #group_first_rows  = np.vstack([pat[0:1]  for pat in patoms])
#         group_second_rows = np.vstack([pat[1:2]  for pat in patoms])
#         group_patoms   = np.vstack([pat[2: , :3] for pat in patoms])

#         num_patoms = len(patoms)
#         avg_rows = int(np.ceil(group_patoms.shape[0] / num_patoms))
...
#             ref_patom_values
#         ))
#         np.save(os.path.join(output, f'patom_{ref_id:.8f}.npy'), ref)
