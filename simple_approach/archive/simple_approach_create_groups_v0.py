...
for i in range(0,10,1):
    ...
    for j in range(0,20,1):
        frame = sequence[j]
        out_patoms = patoms(frame, seq_ind, j)
        holding_set = set()
        for i in out_patoms:
            patom_id = i[0,0]
            # save patoms to disk
            ...
            old_patoms.append(i)
            for j in old_patoms:
                id1, id2, score = compare(i, j)
                # if score above threshold add patom id for old patom j and new patoms i to holding list
                if score < 0.3:
                    holding_set.add(id1)
                    holding_set.add(id2)
                # find out which group j belongs to? dictionary? key new ref patom id value(s) matched patoms 
